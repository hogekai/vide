...
*Updated October 2019 - Corrections & Clarifications (Ref Section 8)*
...
- SIMID: The Secure Interactive Media Interface Definition, the API made available to
...
Open Measurement, VPAID and VMAP specs are excluded from VAST compliance
General
...
Implementation contained in the VAST document should be wrapped in CDATA blocks. The VAST
Note
...
Publisher may originate from a client-side player, or (in some SSAI cases) from the publisher
Implementation server. In the event the request comes from the publisher server extra care must
Note be taken to make sure that the calls are made concurrently with the corresponding
...
The publisher must send requests to the URIs provided in tracking elements;
General however, the publisher is not required to do anything with the response that is
Implementation returned. The response is only to acknowledge an event and to comply with the
Note HTTP protocol. This response is typically a 200 with a 1x1 pixel image in the
...
Publisher same moment in time or as close in time as possible. In particular for a VAST
Implementation response containing a `<Linear>` element, compliance with the IAB Digital Video
Note
...
supports Ad Pods, all ads in the Pod must be played to the best of the player's ability.
Media Player If not supported or the Pod cannot be played, the media player should use the error-
Implementation
tracking URI, if provided, to notify the server.
Note
...
currency* The three-letter ISO-4217 currency symbol that identifies the currency of the
...
Implementation single ad and does NOT define a Pod. Conversely, the Ad sequence identifies the
Note sequence of multiple ads and defines an Ad Pod. See section 3.3.1 for details
about Ad Pods.
#### 3.7.1 UniversalAdId
...
States, or Clearcast in the UK. Some countries may have specific requirements for ad-
tracking programs.
...
Attributes Description
idRegistry* A string used to identify the URL for the registry website where the unique
...
Note: With VAST 4.2 we now allow multiple UniversalAdID elements to be passed in a
VAST response
...
files. Over the years as digital video technology advanced, the media files placed in a VAST
tag have come to include complex files that require API integration. Players not equipped
with the technology to execute such files may be unable to play the ad or execute
interactive components. In ads that require API integration, VAST 4 separates media and
interactive files. While `<MediaFiles>` node focus shifts to the exclusive delivery of media
(video and audio), the dedicated `<InteractiveCreativeFile>` element (see section 3.9.3)
opens opportunities for rendering modern secure interactive components in parallel with
video and audio assets. The dedicated `<Verification>` element allows for measurement
capabilities. Disjoining media and executable files enables a wider range of players to
consume enhanced ads as well as performance improvements.
...
API framework required to execute the file.
...
for execution.
fileSize Optional field that helps eliminate the need to calculate the size based on
...
Publishers consume mezzanine files to transcode the media into a form publisher’s system
and user devices support. The mezzanine file should never be used for the direct ad
playback.
.
...
delivery* Either progressive for progressive download protocols (such as HTTP) or
streaming for streaming protocols.
type* MIME type for the file container. Popular MIME types include, but are not limited
to “video/mp4” for MP4, “audio/mpeg” and "audio/aac" for audio ads.
...
Optional.
Default value = 2D
* VAST tags served to ad-stitching servers require a mezzanine file; server may reject the
...
underlying video or delaying the adStopped call after adVideoComplete. If set to
true, the extension of the duration should be user-initiated (typically, by
...
and Wrapper formats offer the ClickThrough, ClickTracking and CustomClick elements.
These elements are defined in the following sections.
Player Support Required
Required in No
Response
...
viewer clicks the ad. The clickthrough is available in the InLine and Wrapper formats and is
used when the Linear ad unit cannot handle a clickthrough.
Player Support Required
...
altText Alternative text for the image. In an html5 image tag this should be the text for the
...
hoverText Hover text for the image. In an html5 image tag this should be the text for the title
attribute.
...
files that are described in section 3.15. Tracking event elements are described in section
3.14. Ad parameters are used to provide contextual information to the ad and are described
in section 3.8.2.
...
minSuggestedDurati The minimum suggested duration that the creative should be displayed;
on duration is in the format HH:MM:SS.mmm (where .mmm is in milliseconds and
is optional)
...
Required in At least one Companion is required if CompanionAds is provided
Response
...
- interactiveStart: With VAST 4, video playback and interactive creative playback now
happens in parallel. Video playback and interactive creative start may not happen at
the same time. A separate way of tracking the interactive creative start is needed.
The interactive creative specification (SIMID, etc.) will define when this event should
be fired.
...
The URI to an HTML resource file to be loaded into an iframe by the publisher.
Associated with the ad component identified in the parent element, which is either:

```xml
<NonLinear>, <Companion>, or <Icon>.
```
...
A “snippet” of HTML code to be inserted directly within the publisher’s
HTML page code.
Player Support Required for `<Icon>` and for `<NonLinear>` or `<Companion>` when supported
Required in One of `<StaticResource>`, `<IFrameResource>`, or `<HTMLResource>` is required
...
browserOptional* Boolean value. If true, this resource is optimized and able to execute in an
...
| 3 | Error during resource load. The player/SDK was not able to fetch the verification resource, or some error occurred that the player/SDK was able to detect. Examples of detectable errors: malformed resource URLs, |

404 or other failed response codes, request time out. Examples of
potentially undetectable errors: parsing or runtime errors in the JS
resource.
Player Support Required
Required in No
Response
...
Category fields that violate the BlockedAdCategories fields of upstream wrappers (see
section 3.4.5). If an InLine ad is skipped due to a category violation, the client must notify
the ad server using the `<Error>` URI, if provided (error code 205), and move on to the next
option.
Player Support Optional
Required in Response No*
Parent Wrapper
Bounded 0+
Content A string that provides a comma separated list of category codes or labels per
...
fileSize, mediaType
Element Attributes Required Section
/MediaFile id, delivery, type, Yes 3.9.1
bitrate, minBitrate,
maxBitrate, width,
height, scalable,
mantainAspectRatio,
...
Note: In order to ensure that new macros can be added without requiring a new VAST
version at every change, we are maintaining the latest list on the IAB Tech Lab’s VAST
github repository. Please visit
http://interactiveadvertisingbureau.github.io/vast/vast4macros/vast4-macros-latest.html for
the latest list and for more details on the process of adding new macros.
### 6.2 Generic Macros
...
Contexts All tracking pixels
VAST request URIs
Description Indicates the type of ad placement. Refer to the Placement Subtypes - Video
list in AdCOM
Example 1
Name [ADTYPE]
Type string
...
as defined in the adType attribute of the VAST `<Ad>` element. See Section
#### 3.3.2 for details .
...
Support Status Optional
Contexts All tracking pixels
Description Indicates the creative, using UniversalAdId values described in section
3.7.1.
...
Contexts All tracking pixels
VAST request URIs
Description Indicates the maximum length allowed for the ad
break. (in seconds)
...
Contexts All tracking pixels
VAST request URIs
Description Indicates the minimum length of time allowed for the ad break. (in
seconds)
...
Contexts All tracking pixels
VAST request URIs
Description Indicates the maximum number of ads allowed in the ad
break.
...
Contexts All tracking pixels
VAST request URIs
Description Indicates the minimum length allowed for any
individual ad in the ad break. (in seconds)
Example 5 (5 seconds is the min duration of any ad allowed in
the break)
...
Contexts All tracking pixels
VAST request URIs
Description Indicates the maximum length allowed for any
individual ad in the ad break. (in seconds)
Example 15 (15 seconds is the maximum duration of any ad
allowed in the break)
### 6.4 Client Info
...
2 - Server fires request or tracking call on behalf of another server, unknown party, or based on its own decision, without an explicit signal from the client.
...
Contexts All tracking pixels
VAST request URIs
Description The full URL of the top level page where the end user will view the ad. Where
required and applicable, but unknown or unavailable, -1 or -2 must be set, as
described in 6.1. When required and not applicable (such as in an app), 0 must be
set.
Example Unencoded:
...
should not be an app store ID such as iTunes store ID. Where required and
applicable, but unknown or unavailable, -1 or -2 must be set, as described in 6.1.
When required and not applicable (such as in a web context), 0 must be set.
...
Contexts All tracking pixels
VAST request URIs
Description An identifier of the OM SDK integration. This is the same as the “name” and
“versionString” parameters of the OMID Partner object. This will allow creative
providers to determine if the OM partner integration is acceptable.
Note - This value is essential for communicating the certification status of the
OMID integration. If partner ID is not shared, verification vendors will not be
able to properly measure and report on this inventory.
...
Contexts All tracking pixels
VAST request URIs
Description The publisher-specific content identifier for the content asset into which the ad
is being loaded or inserted. Only applicable to in-stream ads.
The format of this field is similar to the [UNIVERSALADID] macro, consisting
of a registry identifier and a registry-specific content identifier. If you are not
using a public registry, you can use your own domain name as the registry
identifier.
If this macro is provided, the provided content identifier should uniquely define
a specific content asset.
Format: registry_id id_value - with a space separating registry_id
and id_value.
Example Unencoded:
...
Contexts All tracking pixels
VAST request URIs
Description The URI of the main media content asset into which the ad is being loaded or
inserted. Only applicable to in-stream ads.
Example Unencoded:
...
Description VAST Error Code. Replaced with one of the error codes listed in section 2.3.6.3
...
The following corrections were made to the VAST 4.2 doc. These did not change the actual
technical spec or XSD, and so did not result in an update to the version.
